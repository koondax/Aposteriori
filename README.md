...
//Subsection%%%%%%%%%%%%DEFINITION: FUNCTIONS %%%%%%%%%%%%%%%%%%%
real mx=16.0, mt=1;
...
func real uexact(real time){
    real out=2-exp((1+mt*time^2)*mx*x*y*(1-x)*(1-y));
    return out;
};
...
{
 real fac= exp((1+mt*time^2)*mx*x*y*(1-x)*(1-y));
 real f= -2*mx*(y*(1-y)+ x*(1-x))*(1+mt*time^2);
 f+= g*mx*(y*(1-x)*(1-y) -x*y*(1-y))*(1+mt*time^2)/fac;
 f-=2*time*mx*mt*x*y*(1-x)*(1-y)/(3*pow(fac, 1.0/3.0));
return f;
};
...
{
real fac= exp((1+mt*time^2)*mx*x*y*(1-x)*(1-y));
return -2*time*mx*mt*x*y*(1-x)*(1-y)/(3*pow(fac, 1.0/3.0));
}
...
varf BilinearForm(u, v)//The main solver
    = int2d(Th)(
      (dSat(uiter) + MS*dt)*u*v + dt*Perm(Sat(uiter))*(grad(v)'*K*grad(u))
)
      + on(1, 2, 3, 4, u=1)
    ;
...
func Flux=Perm(Sat(uiter))*(grad(u) - [g,0]);//Numerical Flux
func LinFl=(Perm(Sat(u))-Perm(Sat(uiter)))*(grad(u) - [g,0]);//Linearization term for flux
...
// local flux equilibration variational form
    ...
    varf a([sga1,sga2,gma],[v1,v2,q]) = int2d(ThLoc)([v1,v2]'*Kinv*[sga1,sga2]
...
errFlux=int2d(Tref)((Kinv*[sigma1,sigma2]+Flux)'*K*(Kinv*[sigma1,sigma2]+ Flux));
...
dtSerr=(PosPart(u-1)-PosPart(uold-1))/(dt*Diff(1));//This is an approximation
QuadT;
...
for(int j=0; j<3; j++)//Integration Using Simpson's 3/8th Rule
{
tt=tn[n-1]+ dtt*j;
...
EstDeg+=int2d(Tref)(grad(dPsi)'*K*grad(dPsi))/6;
dPsi=PosPart(TP(u)-1);
EstDeg+=int2d(Tref)(grad(dPsi)'*K*grad(dPsi))/6;
dPsi=PosPart(.5*(TP(uold)+ TP(u))-1);
EstDeg+=4*int2d(Tref)(grad(dPsi)'*K*grad(dPsi))/6;
...
FileEstFl<<endl; FileEstJumpSL2<<endl; FileEstJumpPsiH1<<endl; 
